...
gulp.task('templates', function() {
  ...
  gulp.src('app/jade/*.jade') //путь к файлу jade
    .pipe(jade({
      locals: YOUR_LOCALS
    }))
...
gulp.task('sass', function () {
  gulp.src('app/css/main.scss') //адрес папки с scss
    .pipe(sass().on('error', sass.logError)) 
    .pipe(gulp.dest('app/css/css/')) //куда сохранять скомпилированное
...
gulp.task('css', function() {
  gulp.src('app/css/*.css') //путь к папке с файлами сss
    .pipe(autoprefixer({
    ...
    .pipe(gulp.dest('app/css/css/')) //куда сохранить после действия
